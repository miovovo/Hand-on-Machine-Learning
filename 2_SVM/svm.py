...
def get_eta_t(t,eta_initial=0.1,decay_rate=0.5):
    return eta_initial/(1+decay_rate*t)
...
def pegasos(train, test, C, T, loss_type="hinge", eval_interval=100, show=False):
    ...
    for t in tqdm.trange(1, T + 1, desc="Train", ncols=80):
        ...
        if loss_type == "hinge":
            ...
            z = y_i *(ops.dot(W.T,x_i)+b)
            if z.asnumpy().item()<1:
                W = W + ops.scalar_mul(eta_t*y_i,x_i)
                b = b + eta_t * y_i
            ...
            z = ops.dot(W.T,x_i)+b
            exp_loss_gradient = -y_i * x_i * ops.exp(-y_i*z)
            exp_b_gradient = -y_i * ops.exp(-y_i*z)

            W = W + eta_t * exp_loss_gradient
            b = b + eta_t * exp_b_gradient'''

        elif loss_type == "log":
            # TODO: 写出log_loss下的梯度更新公式\
            z = y_i *(ops.dot(W.T,x_i)+b)
            sigmoid_z = 1/(1+ops.exp(-z))
            log_loss_gradient = -x_i * (y_i - sigmoid_z)
            log_b_gradient = -(y_i - sigmoid_z)
            W = W + eta_t * log_loss_gradient
            b = b + eta_t * log_b_gradient
